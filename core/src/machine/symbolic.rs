...
// Inner interpreter vs outer interpreter
// outer interpreter is abstracted over types
// in concrete case, outer interpreter is just really "dumb"


// impl CoreMachine<SymbolicIntStack, MemConcreteIntToSymbolicInt, ...> {
//     pub fn exec(&self) -> Vec<Self> {
//         let inst = self.get_next_inst(); // dyn Instruction<SymbolicIntStack, MemConcreteIntToSymbolicInt>;
//         let exec_rec = self.get_next_inst().exec(); // ExecRecord<SymbolicIntStack, MemConcreteIntToSymbolicInt>;
//     }
// }
