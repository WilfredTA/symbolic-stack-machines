...
pub mod simple_lang {
    use symbolic_stack_machines_core::{instructions::{AbstractInstruction, EnvExtension, ConcreteAbstractExecRecord}, memory::Mem, stack::Stack};
    ...
    > for SimpleLang<T>
    where
    T: std::ops::Add + std::ops::Add<Output = T> + Clone + std::ops::Sub + std::ops::Sub<Output = T> + std::fmt::Debug,
    S: Stack<StackVal = T>,
    M: Mem,
    Extension: EnvExtension,
    {
        fn exec(&self, stack: &S, mem: &M, ext: &Extension) -> symbolic_stack_machines_core::instructions::InstructionResult<ConcreteAbstractExecRecord<S, M, Extension::DiffRecordType>> {
            match self {
